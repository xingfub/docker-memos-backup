...
def __uploadLocal(localfile):
    ...
    # 要上传文件的本地路径
    try:
        key_ = "memosBack/" + __getSqlFileName()
        print("key", key_)
    ...
    try:
        if os.path.exists("./config.txt"):
            with open("./config.txt", "r") as file:
                lines = file.readline()
                obj_ = json.loads(lines);
                __doNetCacheDelNet(obj_["key"])
        with open("./config.txt", "w") as file:
            file.write(json.dumps({"key": key_, "url": url_}))
    except Exception as err:
        print(err)
...
if __name__ == "__main__":
    print("main")
    doMemos(True)
    localfile="/home/ubuntu/light/memos/.memos/memos_prod.db"
    key = __uploadLocal(localfile)
    __doNetCache(key, __getUrl(key))
    doMemos(False)
